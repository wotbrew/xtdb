...
(defrecord LMDBKv [db-dir env env-flags dbi ^StampedLock mapsize-lock sync? cp-job ^AtomicInteger mapsize-increase-ctr]
  kv/KvStore
  (store [this kvs]
    (loop [mapsize-increases (.get mapsize-increase-ctr)]
      (when-not (cursor-put-if-map-not-full mapsize-lock env dbi kvs)
        (if (.compareAndSet mapsize-increase-ctr mapsize-increases (inc mapsize-increases))
          (binding [*mapsize-increase-factor* (* 2 *mapsize-increase-factor*)]
            ...
            (kv/store this kvs))
          (recur (.get mapsize-increase-ctr))))))
  ...
  (let [env-flags (or env-flags
    ...
    (try
      ...
      (let [kv-store (map->LMDBKv {:db-dir db-dir
                                   ...
                                   :sync? sync?
                                   :mapsize-increase-ctr (AtomicInteger. 0)})]
        (cond-> kv-store
